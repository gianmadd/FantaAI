...
DATA_RAW_PATH_SPECIFIC = os.path.abspath(
    f"../../data/{PROVIDER_NAME}/raw/specifics/players/2020"
)
...
def get_static_useful_fields_json(data):

    # Extract the name of the key from the "get" field
    ...
    # Populate the dynamic key with items from the response
    for item in data.get("response", []):
        cleaned_data[field_name].append(item)
    ...
    def flatten_fields(prefix, obj):
        ...
        for key, value in obj.items():
            if isinstance(value, dict):  # Recursively flatten dictionaries
                flat_data.update(
                    flatten_fields(f"{prefix}_{key}" if prefix else key, value)
                )
...
def clean_json(input_path, output_path):
    ...
    try:
        ...
        cleaned_data = get_player_useful_fields_json(data)
